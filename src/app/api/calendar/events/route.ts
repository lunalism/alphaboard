 ...
 * @query category - 카테고리 필터 (institution, earnings, corporate, crypto, options, dividend, holiday, 선택)
 *
 * @description
...
function isValidCategory(category: string): category is EventCategory {
  const validCategories: EventCategory[] = ['institution', 'earnings', 'corporate', 'crypto', 'options', 'dividend', 'holiday'];
    ...
    return NextResponse.json(
      {
        ...
        message: `유효하지 않은 카테고리입니다. 가능한 값: institution, earnings, corporate, crypto, options, dividend, holiday`,
      },
      { status: 400 }
    );
  }
